...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      let response = client.post("/login", formData);
        ...
        <div>
          ...
          <div className="mt-8">
            <form className="w-full max-w-md" onSubmit={handleLogin}>
              ...
              <button
                type="submit"
                className="mt-8 block w-full px-4 bg-gray-300 py-3 border border-gray-300 rounded-md text-sm font-semibold hover:bg-green transition all-ease duration-300"
              >
                sign up
              </button>
          ...
          <div className="lg:w-[75%] m-auto p-8 img h-[33rem] rounded">
            <div className="pt-[22rem]">
              <p className="text-[10px] text-center text-white">
                Recent global estimates by UNICEF and partners indicate that at
                least 340 million children under 5 (one in two) suffer from
                hidden hunger. In the Western Africa UN sub-region 67% of
                children under 5 suffer from hidden hunger.
              </p>
            </div>
          </div>
        </div>
